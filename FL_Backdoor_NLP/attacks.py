...
def hotflip_attack(averaged_grad, embedding_matrix, grad_embedding_on_clearn_data, trigger_token_ids, original_trigger_token_ids,
    ...
    print('size:------>',averaged_grad.size(),embedding_matrix.size())
    gradient_dot_embedding_matrix = torch.einsum("bij,kj->bik",
                                                 (averaged_grad, embedding_matrix))
    grad_embedding_on_clearn_data = grad_embedding_on_clearn_data.cpu()
    print('grad_embedding_on_clearn_data.size()',grad_embedding_on_clearn_data.size())
    grad_embedding_norm_on_clearn_data = torch.norm(grad_embedding_on_clearn_data,dim=1).unsqueeze(0).unsqueeze(0)

    grad_embedding_on_clearn_data_dis = torch.norm(grad_embedding_on_clearn_data-grad_embedding_on_clearn_data[original_trigger_token_ids,:],dim=1).unsqueeze(0).unsqueeze(0)
    print('grad_embedding_on_clearn_data_dis.size()',grad_embedding_on_clearn_data_dis.size())
    # yuyuyuy

    if not increase_loss:
        gradient_dot_embedding_matrix *= -1    # lower versus increase the class probability.
        print(gradient_dot_embedding_matrix.size())
        gradient_dot_embedding_matrix = 3.0*gradient_dot_embedding_matrix -1.0*grad_embedding_norm_on_clearn_data
        - 1.0*grad_embedding_on_clearn_data_dis
        print(gradient_dot_embedding_matrix.size())
