    ...
    for participant_id in sampled_participants:
        ...
        if helper.params['is_poison'] and participant_id in helper.params['adversary_list'] and trained_posioned_model_weights is None:
            ...
            try:
                ...
                for internal_epoch in range(helper.params['retrain_poison']):
                    if helper.params['model'] == 'resnet':
                        if helper.params['dataset'] == 'cifar10':
                            loss = train_cifar10_resnet_poison(helper, model, poison_optimizer, criterion, mask_grad_list, global_model_copy)
        ...
        # Only one attacker trains. The other attackrs just copy the trained model
            ...
            for internal_epoch in range(helper.params['retrain_no_times']):
                ...
                if helper.params['model'] == 'resnet':
                    if helper.params['dataset'] == 'cifar10':
                        loss = train_cifar10_resnet(helper, model, optimizer, criterion, participant_id, epoch)
...
def train_cifar10_resnet_poison(helper, model, poison_optimizer, criterion, mask_grad_list, global_model_copy):

    for inputs, labels in helper.poisoned_train_data:

        for pos in range(labels.size(0)):
            labels[pos] = helper.params['poison_label_swap']
...
def train_cifar10_resnet(helper, model, optimizer, criterion, participant_id, epoch):
